...
from shejane_runtime.plugins.runtime_assets import RuntimeAssetStore  # noqa: E402
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="shejane-ffmpeg-release-") as temporary:
        installed = RuntimeAssetStore(Path(temporary)).install(asset)
        if (
            args.expected_canonical_digest
            and installed.digest != args.expected_canonical_digest
        ):
