...
def main() -> None:
    ...
    parser.add_argument("--ffmpeg-runtime-asset-digest", required=True)
    parser.add_argument("--whisper-runtime-asset-digest", required=True)
    parser.add_argument("--version", default="0.1.0")
    parser.add_argument("--worker", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    for name in ("ffmpeg_runtime_asset_digest", "whisper_runtime_asset_digest"):
        if not re.fullmatch(r"sha256:[0-9a-f]{64}", getattr(args, name)):
            parser.error(f"--{name.replace('_', '-')} must be a canonical SHA-256 digest")
    if not re.fullmatch(
    ...
    entrypoint = (
        "payload/speech-worker.exe"
        if args.platform.startswith("windows/")
        else "payload/speech-worker"
    )
    ...
    for path in worker.rglob("*"):
        ...
        if not (stat.S_ISDIR(metadata.st_mode) or stat.S_ISREG(metadata.st_mode)):
            parser.error("--worker contains an unsafe entry")
    with tempfile.TemporaryDirectory(prefix="speech-plugin-package-") as temporary:
        ...
        manifest = (
            ...
            .replace(
                "__FFMPEG_RUNTIME_ASSET_DIGEST__",
                args.ffmpeg_runtime_asset_digest,
            )
            .replace(
                "__WHISPER_RUNTIME_ASSET_DIGEST__",
                args.whisper_runtime_asset_digest,
            )
        )
        if "__" in manifest:
