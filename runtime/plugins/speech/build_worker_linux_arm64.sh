...
temporary="$(mktemp -d)"
image="shejane-speech-worker-builder:local-$$"
cleanup() {
  rm -rf "$temporary"
...
docker build --pull --platform linux/arm64 \
  --file "$root/plugins/speech/worker/Dockerfile.linux-arm64" \
  --tag "$image" \
  "$root/plugins/speech/worker"
  ...
  docker run --rm --platform linux/arm64 --network none \
    ...
    sh -lc '
      ...
      pyinstaller --clean --noconfirm --onedir \
        ...
        --specpath /tmp \
        /src/plugins/speech/worker/speech_worker.py
    '
}

build_worker "$temporary/first"
build_worker "$temporary/second"
diff -qr "$temporary/first/speech-worker" "$temporary/second/speech-worker"
cp -R "$temporary/first/speech-worker" "$output"
