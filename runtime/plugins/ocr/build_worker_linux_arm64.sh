...
temporary="$(mktemp -d)"
image="shejane-ocr-worker-builder:local-$$"
cleanup() {
  rm -rf "$temporary"
...
docker build --pull --platform linux/arm64 \
  --file "$root/plugins/ocr/worker/Dockerfile.linux-arm64" \
  --tag "$image" \
  "$root/plugins/ocr/worker"
  ...
  docker run --rm --platform linux/arm64 --network none \
    ...
    sh -lc '
      ...
      pyinstaller --clean --noconfirm --onedir \
        ...
        --specpath /tmp \
        /src/plugins/ocr/worker/ocr_worker.py
    '
}

build_worker "$temporary/first"
build_worker "$temporary/second"
diff -qr "$temporary/first/ocr-worker" "$temporary/second/ocr-worker"
cp -R "$temporary/first/ocr-worker" "$output"
