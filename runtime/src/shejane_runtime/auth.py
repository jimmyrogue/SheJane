...
Compatible shape with the Node runtime:
...
class PairingTokenAuthMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        expected = self.token
        if not expected:
            # Runtime started without a token configured — refuse everything
            # except /v1/health. Lets the Electron host detect "not yet paired".
