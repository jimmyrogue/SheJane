"""Contract tests for `DELETE /v1/memory`.
...
in `runtime/sdk/src/client.ts` and verifies the underlying
...
def test_clear_memory_idempotent(client: TestClient) -> None:
    ...
    resp1 = client.delete("/v1/memory", headers=HEADERS)
    resp2 = client.delete("/v1/memory", headers=HEADERS)
