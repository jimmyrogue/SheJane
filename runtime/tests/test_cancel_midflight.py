"""End-to-end cancel test — proves `POST /v1/runs/:id/cancel`
...
def slow_client(monkeypatch) -> TestClient:
    ...
    monkeypatch.setattr(
        "shejane_runtime.llm.fake.FakeBackendChatModel._astream",
        _slow_astream,
    )
...
def test_cancel_interrupts_running_run(slow_client: TestClient) -> None:
    ...
    # Start the run
    r = slow_client.post("/v1/runs", headers=headers, json=run_command("slow"))
    ...
    def cancel_after_delay() -> None:
        ...
        resp = slow_client.post(f"/v1/runs/{run_id}/cancel", headers=headers)
    ...
    with slow_client.stream("GET", f"/v1/runs/{run_id}/stream", headers=headers) as resp:
    ...
    r = slow_client.get(f"/v1/runs/{run_id}", headers=headers)
    assert r.status_code == 200
