...
6. **Model selection is a catalog, not fast/deep tiers.** Client requests send `model: "auto"` or a concrete catalog model id. `Auto` is resolved by the Go API against enabled chat models and emits `model.selected`; the daemon and web loop must not reintroduce their own fast/deep classifiers. The stable model id is still stored in `model_configs.slot` for migration safety, but docs and UI call it "model ID". Text model billing uses input/output token multipliers relative to DeepSeek Pro = 1, with legacy `credit_multiplier` only as a fallback; the global markup remains the margin knob. Image models are not shown in the chat picker; the current image resolver only supports `image.default`.
