...
生产采用「CI 构建镜像 → 服务器拉取运行」的模式，不在服务器上从源码构建。Runtime、Desktop、Cloud、Admin 和 Runtime SDK 拥有独立版本。

**发布镜像（在开发机）**

```bash
make release COMPONENT=cloud VERSION=0.1.0
make release COMPONENT=admin VERSION=0.1.0
```

会校验工作区干净且当前在 `main`，然后推送对应模块标签。`cloud-v*` 和 `admin-v*` 只触发各自的多架构镜像发布：

- `ghcr.io/jimmyrogue/shejane-cloud:0.1.0`（同时打 `latest`）
- `ghcr.io/jimmyrogue/shejane-admin:0.1.0`（同时打 `latest`）

Cloud 和 Admin workflow 在镜像推送后按 digest 做供应链守门：

- Trivy 扫描镜像 OS/library 漏洞；未修复的 HIGH / CRITICAL 漏洞会让 workflow 失败，阻止继续签名和发布 SBOM。
- Cosign 使用 GitHub Actions OIDC 对发布 digest 做无密钥签名。

Admin 使用空的 `VITE_API_BASE_URL` 构建，通过同源 `/api/*` 访问 Cloud。Runtime 使用 `runtime-v*` 发布四组自包含压缩包与 SHA-256 校验文件；Desktop 使用 `desktop-v*` 发布，并严格下载 `apps/desktop/runtime-version.json` 锁定的 Runtime；`runtime-client-v*` 发布 npm SDK。
...
#    APP_DOMAIN / ADMIN_DOMAIN、CLOUD_IMAGE_VERSION 和 ADMIN_IMAGE_VERSION。
cp .env.example .env
...
> - `CLOUD_IMAGE_VERSION` 和 `ADMIN_IMAGE_VERSION` 分别钉到具体版本；否则会使用 `latest`。

常用：

```bash
make deploy CLOUD_IMAGE_VERSION=0.1.0 ADMIN_IMAGE_VERSION=0.1.0
...
- `POSTGRES_PASSWORD` 已从默认 `shejane` 改掉；`CLOUD_IMAGE_VERSION` 和 `ADMIN_IMAGE_VERSION` 已分别钉到具体版本。
