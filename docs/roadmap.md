...
- 模型选择已经切到 **Auto 意图 + 后台模型目录**。用户端始终有 `Auto`，并提供 `更快` / `更强` 两个 Auto 意图（wire 值 `auto.fast` / `auto.smart`），具体模型页按厂商显示后台启用的 chat 模型。管理员用「模型 ID + 显示名 + 厂商 + 能力档位 + provider_kind + key + base_url + model_name + 输入/输出 token 费率」配置模型；数据库字段仍叫 `slot`，只是历史字段名。
