...
- [ ] ⚠️ **接通充值入口**(功能 · 高 · M)—— `createSubscriptionCheckout` 后端已就绪,**零 UI 调用方**;余额耗尽提示"请充值"却没有任何按钮。
- [ ] ⚠️ **修欢迎页 4 个死磁贴**(功能 · 高 · S)—— 建议磁贴无 `onClick`,新用户首屏第一个交互就点不动。
- [ ] **安全/计费三连**(后端 · 高 · M,一批做):
  - [ ] 全站限流(auth 防爆破 / 计费端点防成本放大 / webhook 防洪)—— 现零限流
  - [ ] ⚠️ 修"空 LLM key 静默回退 Mock 仍计费"—— 返回假文本却照常结算积分
  - [ ] 图片网关出站抓取加 SSRF 防护 —— `fetchURLBytes` 无私网过滤,可打内网元数据
- [ ] **上线监控/告警/日志聚合**(运维 · 高 · L)—— 最大盲区;先上 metrics + Sentry + `/health` 探活 + 日志外送。
- [ ] **数据安全兜底**(运维 · 高 · M+S):
  - [ ] 把仓库工作区里的 `backup-*.sql.gz` 移走(DB dump 躺开发机 = 数据泄露)— 几乎零成本,先做
  - [ ] 自动备份 + 自动异地拷贝 + 定期恢复演练(现 100% 手动、无 cron、无异地)
- [ ] **Stripe webhook 改原子事务 + 处理退款/争议**(后端 · 高 · M)—— 现 record→process→mark 三段独立调用,崩在中间会重处理;退款用户仍保留积分。
...
后端/运维:
- [ ] `/readyz` 真就绪探针(真正 ping DB;现 `/health` 是静态桩)(中 · S)
...
- [ ] api/client/admin/caddy 加 compose healthcheck(现只有 postgres 有)(中 · S)
