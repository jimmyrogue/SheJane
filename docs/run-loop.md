...
> **状态**：截至 `refactor/infrastructure` 分支 head（106 测试全过）
...
╚════════════════════════════════════════════════════════════════════════════════════════════╝
  ...
  │   │      │     → llm.token / llm.reasoning / llm.tool_call_chunk /          │    │
  │   │      │       tool.end / subagent.spawned / subagent.completed /         │    │
  │   │      │       graph.node / agent.custom                                   │    │
  │   │      ▼                                                                   │    │
  │   │   store.append_event  +  queue.put_nowait                                │    │
  │   │      │                          │                                       │    │
  │   │      ▼                          ▼                                       │    │
  │   │  SQLite 持久化            sse-starlette 推流                              │    │
  ...
  │     ├─ snapshot.next 非空（卡在 interrupt）                                          │
  │     │     state = waiting_permission                                                │
  │     │     event: run.waiting { next, interrupts[].value }                          │
  │     │     等 POST /resume 或 /permissions/:id → coordinator.resume_run             │
  │     │       → agent.ainvoke(Command(resume=decision))  → 回到 ❷ 主循环             │
  │     └─ snapshot.next 空                                                             │
  │           state = completed                                                         │
...
| **流式 token** | `messages` 模式 → `llm.token` | `test_streaming_latency` ✅ **p50 24.8ms** |
