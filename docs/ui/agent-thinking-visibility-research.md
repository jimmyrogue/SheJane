...
这是对现有信息架构的收敛，不需要改变 Runtime 事件协议，也不需要删除审计数据。先在 Desktop 投影视图中做确定性分组即可；只有当现有事件缺少目标、阶段或状态字段时，才需要补协议。
