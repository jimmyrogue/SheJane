...
- The sidebar hierarchy is the logo, `新对话`, and the conversation list. The bottom footer keeps only `插件` and `设置`; `Skill`, `MCP`, and installed plugins live as quiet secondary tabs inside the plugin workspace.
