...
> 研究日期：2026-07-18
>
> 范围：Electron/React Desktop、FastAPI/LangGraph Runtime、Runtime SDK、内置 Tool、MCP、Plugin/Managed Worker、SSE 与持久恢复。
>
...
状态所有者：Runtime 的 Tool Receipt 与等待候选；Desktop 只持有投影
...
- [`runtime-tools.contract.test.ts`](../apps/desktop/src/shared/local-host/runtime-tools.contract.test.ts) 已经把 `/local/v1/tools` 与已执行 Tool 列表做反向相等检查。新增公开 Tool 没有 E2E 路径时会失败；这条 inventory gate 应保留。
- [`runtime-agent.contract.test.ts`](../apps/desktop/src/shared/local-host/runtime-agent.contract.test.ts) 已经纵向覆盖附件、Skill、Subagent、Todo、一次权限批准、一次结构化问答和运行中取消。
- [`client.contract.test.ts`](../apps/desktop/src/shared/local-host/client.contract.test.ts) 已经检查命令幂等冲突、SSE envelope、durable cursor replay 和错误形状。
- [`test-packaged-desktop-runtime.mjs`](../scripts/test-packaged-desktop-runtime.mjs) 已经验证 macOS/Windows 打包应用能拉起 Runtime、交付 loopback token 并在退出时关闭子进程；macOS arm64 还先由包内冻结 Runtime 对 VM manifest 和全部资产做生产 preflight，避免“路径注入成功但资产 schema 已过期”的假绿。
...
- MCP 配置漂移 E2E 实际发现了“等待批准时删除并重建 server，旧批准会静默执行新 server”的 P10 身份缺陷。MCP Tool version 现在绑定非秘密配置 fingerprint；旧 receipt 与新实现不一致时 Run 以 `tool_receipt_conflict` 安全失效且零 Tool 副作用，新 Run 才能使用新配置。Plugin 也已用 v1 等待审批、期间更新到新增拒绝 capability 的 v2、旧 Run 成功且新 Run 失败，证明接纳版本冻结。Skill 现在绑定完整发现树内容指纹：旧 Run 等待期间变更后在 Tool 执行前安全失败，新 Run 使用新版本，checkpoint fork 仍继承源 Run 绑定。Desktop 可见配置冻结也已补齐：旧 Run 完成子代理，新 Run 禁止 `task`；该场景实际发现并修复了关闭开关后默认 `general-purpose` 子代理仍可用的问题。
...
- SDK black-box 现在通过真实 TCP 代理分别制造 SSE 帧中途 half-close 与 RST，断言 incomplete/error、精确 cursor replay、seq 单调无重复和唯一终态；Electron 也通过代理主动 half-close 一次，并证明页面不重载即可自动续流。该测试实际复现并修复了 Desktop 忽略 `completed:false`、永久停在旧批准栏的缺陷。
...
### 1. Desktop 用户关键路径已进入真实窗口门禁

当前 `make test-e2e` 已通过 Playwright 启动真实 Electron，并覆盖 13 条关键路径：启动/聊天/重启持久、SSE 断线续传、Tool approve/deny、`user.ask`、retry/repair CTA、外链 allowlist、系统 clipboard、真实 PPTX、workspace 恢复、设置冻结，以及 Runtime 真实进程崩溃后无需窗口重载即主动呈现离线、同 data dir 重启后恢复在线和新任务。该 crash 流程同时断言被杀 PID 就是真实 health endpoint 所有者，避免只杀启动包装进程形成假故障；它还实际发现并修复了旧 Run 进入 `cleanup_required` 后全局发送状态仍把新对话锁成“停止生成”的跨 thread 泄漏。真实 quit 后的 bundled Runtime 清理由 packaged smoke 负责：它触发应用正常 quit、等待 Desktop 退出，再等待 daemon PID 消失；不在开发态外接 Runtime 的窗口套件重复一个更弱版本。
...
- 事件到了但 Desktop 窗口重载，随后 snapshot 是否收敛到相同结果。
...
| integration | FastAPI/LangGraph/store/worker 进程内或单子系统 | 故障注入、罕见内部 branch、数据库迁移 | Desktop 用户流程 |
| Runtime black-box E2E | 真实 Runtime 进程 + 公共 HTTP/SSE + SDK | P1-P12 协议、Tool/HITL/恢复、持久状态 | Renderer 可用性 |
| Desktop E2E | Electron Main + Renderer + 真实 Runtime | 用户能否完成聊天、审批、问答、失败恢复、设置 | 第三方互联网可靠性 |
...
```text
apps/desktop/e2e/                 # Playwright Electron 用户流程
apps/desktop/src/**.contract.test.ts  # 现有 Runtime SDK 黑盒测试
services/runtime/tests/           # 进程内 integration/fault policy
services/runtime/tests/fixtures/  # fake LLM、MCP、Plugin/Worker 故障 fixture
```
...
7. Desktop quit -> Runtime/worker 子进程全部退出；Runtime crash -> Desktop 明确离线并可重启。
...
- 时间相关 UI 使用 Playwright Clock；它可固定/快进 `Date`、timer 和 animation frame：[Playwright Clock](https://playwright.dev/docs/clock)。Runtime lease/TTL 仍使用 injectable clock 的 integration test，不能假装浏览器 clock 会改变 daemon 时间。
...
| 触发 | 套件 | 目标时长 | 环境 |
|---|---|---:|---|
| 每个 PR | unit + integration + Runtime black-box E2E + 13 条 Desktop critical flows | 10-15 分钟 | Ubuntu；scripted LLM；无外网 |
| 每晚 | resilience + 完整 Tool edge matrix + MCP conformance + Plugin crash | 30-60 分钟 | Ubuntu；可并行分片 |
| 每晚/合并后 | Desktop OS matrix | 20-40 分钟 | macOS/Windows/Linux，按平台能力条件化 |
...
- 修正文档措辞：区分 Runtime black-box E2E、Desktop E2E、integration 和 packaged smoke。
...
- 完成 13 条 Desktop critical flows。
...
- [x] `/local/v1/tools` 每个 Tool 都有 family/risk/traits、真实成功或 guarded-failure 路径；目录中所有声明 required input 的 Tool 都由 schema 反向门禁删除一个必填项，证明 permission 前、attempt 0 失败；无 required input 的 Tool 显式归为 not-applicable。
...
- [x] SSE partial frame/reconnect/multi-subscriber/slow consumer/restart 完成（含 partial frame、half-close、RST、cursor replay、multi-subscriber、slow consumer、Desktop 自动续流，以及首个 `llm.delta` 后进程重启的安全 snapshot 收敛）。
...
- [x] Desktop 13 条 critical flows 用真实 Electron 窗口完成。
...
- [x] macOS/Windows/Linux packaged smoke 以实际支持能力分类（macOS arm64 VM + Desktop、macOS x64/Windows x64 Desktop lifecycle、Linux arm64 packaged Runtime+cgroup/systemd；没有 Linux Desktop 时明确不声称）。
