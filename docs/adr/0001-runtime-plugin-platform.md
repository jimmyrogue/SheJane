...
- 状态：Accepted
- 日期：2026-07-15
- 决策范围：Runtime、Runtime SDK、Desktop、插件能力包
...
- 任意插件 WebView、前端脚本或自定义 Desktop UI。
...
- P2：Desktop 将 `@插件` 和 `/插件:命令` 编译为结构化 Command 字段。
...
- Desktop 只保存 Runtime 投影与待提交命令，不拥有插件安装真相。

### 5.4 被替换的旧路径

当前旧路径由 `services/runtime/local_host/agent/builder.py` 直接装配静态工具和 MCP 目录，Office 工具由 `services/runtime/local_host/tools/registry.py` 静态注册，并在 tool visibility 中存在 Office 专属规则。
...
Desktop 左侧能力区新增“插件”Tab，使用 SheJane 现有视觉系统。它只渲染 Runtime 返回的可信结构化数据。
