...
| 7 | 插件来源与开发者发布 | 可自托管来源、开发者规范和发布检查完整 |
...
## 13. Phase 7：插件来源与开发者发布

### 13.1 插件来源

先定义可自托管的只读、签名、静态来源格式：
...
SheJane 项目可以提供一个默认来源，但它与用户配置的其他来源使用相同协议，没有隐藏权限。v1 手动安装或更新；不做后台自动更新。
...
- 已冻结并实现 [`source-index-v1.md`](../plugins/source-index-v1.md) 的静态来源验证：来源公钥必须由用户在索引外配置，Runtime 对精确 index bytes 校验 SHA-256 与 Ed25519；包 URL 仅允许 HTTPS，WASI/Managed Worker 平台、唯一版本目标、权限与 MIME 摘要均严格校验。
- Runtime 已用 `plugin_sources` 持久化 last-known-good 索引，并通过 `plugin.source.add/refresh/install/remove` 接入既有幂等 Command 日志。网络读取只允许逐跳复核的 HTTPS，限制重定向、超时、索引、签名与包大小；刷新验证完成前不替换旧索引。
- Runtime SDK 与 Plugin Tab 已支持来源列表/详情、添加、手动刷新、精确包安装和移除。来源安装冻结 source revision、plugin/version/execution/platform/package digest，下载后再次核对归档大小、canonical package digest、受信发行者签名以及 manifest 与索引摘要；重放同一命令不会重复下载。移除来源不删除已安装插件。
- v1 不做后台刷新、自动安装或自动更新；开发者仍需自行托管静态 HTTPS 索引和包。默认来源及发布自动化尚未提供。
...
- `docs/operations.md`：安装、未签名包策略、插件来源、签名、存储、清理和排障。
