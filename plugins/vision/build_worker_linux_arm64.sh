...
temporary="$(mktemp -d)"
image="shejane-vision-worker-builder:local-$$"
cleanup() {
  rm -rf "$temporary"
...
docker build --pull --platform linux/arm64 \
  --file "$root/plugins/vision/worker/Dockerfile.linux-arm64" \
  --tag "$image" \
  "$root/plugins/vision/worker"
  ...
  docker run --rm --platform linux/arm64 --network none \
    ...
    sh -lc '
      ...
      pyinstaller --clean --noconfirm --onedir \
        ...
        --specpath /tmp \
        /src/plugins/vision/worker/vision_worker.py
    '
}

build_worker "$temporary/first"
build_worker "$temporary/second"
diff -qr "$temporary/first/vision-worker" "$temporary/second/vision-worker"
cp -R "$temporary/first/vision-worker" "$output"
