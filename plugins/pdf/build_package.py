...
def main() -> None:
    ...
    entrypoint = "payload/pdf-worker.exe" if args.platform.startswith("windows/") else "payload/pdf-worker"
    ...
    for path in worker.rglob("*"):
        ...
        if not (stat.S_ISDIR(metadata.st_mode) or stat.S_ISREG(metadata.st_mode)):
            parser.error("--worker contains an unsafe entry")
    with tempfile.TemporaryDirectory(prefix="pdf-plugin-package-") as temporary:
