...
# inside local-host/python. Formats + lints so Claude sees its own
...
case "$file" in
  *"$ROOT_DIR/local-host/python/"*|"local-host/python/"*) ;;
...
relpath="${file#"$ROOT_DIR/local-host/python/"}"
relpath="${relpath#local-host/python/}"
