...
description: Review code changes for daemon ↔ TS client ↔ Go API contract drift. Reads pydantic models, generated TS types, hand-written client.ts, and matching Go handlers; reports any field/shape/enum disagreement between layers. Use proactively after any change to api_schemas.py, server.py response_model, generated.ts, client.ts, or api/internal/httpapi/. Read-only — does not modify files.
...
- **Client (generated)**: `packages/runtime-client/src/generated.ts` — produced by `make schemas` from the daemon's openapi.json. Must NOT be hand-edited.
- **Client (hand-written)**: `apps/desktop/src/shared/local-host/client.ts` re-exports from `generated.ts`. Hand-written types (DesktopBridge, LocalHostConfig, AgentRunEvent) are documented at the top of the file.
...
- A pydantic field was renamed or removed → was `generated.ts` regenerated? Run `git status -- packages/runtime-client/src/generated.ts` to check.
...
|---|---|---|---|
| ❌ break | daemon ↔ TS gen | `LocalRun.canceled_at` added but `generated.ts` not regenerated | Run `make schemas` and commit |
