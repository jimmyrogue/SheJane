...
1. `configure_logging()` sets up structlog once at daemon boot. All module
   logs (loggers obtained via `structlog.get_logger(__name__)`) flow
   through the same JSON/console renderer.

2. `DaemonObserver(AsyncCallbackHandler)` hooks into LangChain's callback
   ...
   for cross-correlation with the daemon's other logs.
...
def build_callbacks() -> list[AsyncCallbackHandler]:
    ...
    Always includes `DaemonObserver`. Conditionally appends Langfuse if the
    vendor SDK is installed and credentials are present.
    """
    if is_disabled():
        return []

    callbacks: list[AsyncCallbackHandler] = [DaemonObserver()]
    ...
    if os.environ.get("LANGFUSE_PUBLIC_KEY") and os.environ.get("LANGFUSE_SECRET_KEY"):
        try:
            ...
            structlog.get_logger("local_host.observability").warning(
...
class DaemonObserver(AsyncCallbackHandler):
