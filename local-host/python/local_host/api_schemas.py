...
class CreateRunRequest(BaseModel):
    ...
    # cloud LLM endpoint, which owns resolution — "auto"/""/an unknown id map
    # to the default (highest-priority) model. Concrete provider/model names
    # live in the Go model registry, never here. Default "auto" so direct API
    # callers (tests, curl) get the default model. (Replaces the old fast/deep
    # "mode"; "pro"/"deep"/"fast" still flow through as legacy aliases.)
    model: str = "auto"
...
# ---------------------------------------------------------------------------
# Lark connector and local todos
# ---------------------------------------------------------------------------


LarkConnectionStatus = Literal["disconnected", "needs_auth", "connected", "error"]
LarkAuthMode = Literal["lark_cli"]


class LocalLarkConnection(BaseModel):
    id: str
    provider: Literal["lark"] = "lark"
    status: LarkConnectionStatus
    tenant_label: str = ""
    account_label: str = ""
    auth_mode: LarkAuthMode = "lark_cli"
    cloud_extraction_enabled: bool = True
    data_retention_days: int = Field(default=7, ge=1, le=30)
    auto_sync_enabled: bool = False
    auto_sync_interval_minutes: int = Field(default=5, ge=1, le=60)
...
class LocalLarkConnectorStatus(BaseModel):
    available: bool
    source: Literal["bundled", "system", "missing"]
    executable_path: str | None = None


class LocalLarkStatus(BaseModel):
    connection: LocalLarkConnection
    connector: LocalLarkConnectorStatus


class LocalLarkConnectResponse(BaseModel):
    connection: LocalLarkConnection
    connector: LocalLarkConnectorStatus
...
class UpdateLocalLarkConnectionRequest(BaseModel):
    cloud_extraction_enabled: bool | None = None
    data_retention_days: int | None = Field(default=None, ge=1, le=30)
    auto_sync_enabled: bool | None = None
    auto_sync_interval_minutes: int | None = Field(default=None, ge=1, le=60)


LarkSourceType = Literal["p2p", "group", "thread"]


class LocalLarkSource(BaseModel):
    id: str
    connection_id: str
    provider_source_id_hash: str
    source_type: LarkSourceType
    display_label: str = ""
...
class ListLocalLarkSourcesResponse(BaseModel):
    sources: list[LocalLarkSource]


class UpdateLocalLarkSourceRequest(BaseModel):
    display_label: str | None = None
    sync_enabled: bool | None = None


TodoPriority = Literal["now", "today", "later", "fyi"]
TodoStatus = Literal["open", "completed", "dismissed", "snoozed"]
...
class LocalTodoItem(BaseModel):
    ...
    title: str
    summary: str = ""
    suggested_action: TodoSuggestedAction = "none"
    due_at: str | None = None
    confidence: float = 0
    extraction_provider: TodoExtractionProvider = "rules"
    evidence_preview: str = ""
    created_at: str
    updated_at: str


class ListLocalTodosResponse(BaseModel):
    todos: list[LocalTodoItem]


class UpdateLocalTodoItemRequest(BaseModel):
    priority: TodoPriority | None = None
    status: TodoStatus | None = None
    due_at: str | None = None


class QuoteLocalTodoRequest(BaseModel):
    include_evidence: bool = True


class QuoteLocalTodoResponse(BaseModel):
    todo_id: str
    text: str


class SyncLocalLarkRequest(BaseModel):
    limit: int = Field(default=100, ge=1, le=500)
    extraction_provider: TodoExtractionProvider = "cloud_redacted"
    model: str = "auto"


class SyncLocalLarkResponse(BaseModel):
    provider: Literal["lark"] = "lark"
    extraction_provider: TodoExtractionProvider = "cloud_redacted"
...
class PreviewLocalLarkRequest(BaseModel):
    limit: int = Field(default=100, ge=1, le=500)


class LocalLarkPreviewCandidate(BaseModel):
    message_id: str
    source_id: str
    source_label: str = ""
    source_type: LarkSourceType
...
class PreviewLocalLarkResponse(BaseModel):
    provider: Literal["lark"] = "lark"
    processed_messages: int
    candidate_count: int
    skipped_messages: int
    candidates: list[LocalLarkPreviewCandidate]


class ClearLocalLarkCacheResponse(BaseModel):
