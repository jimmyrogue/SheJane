...
recent_llm_errors() {
  cd "$ROOT_DIR"
  docker compose -f infra/cloud/docker-compose.yml exec -T postgres psql -U shejane -d shejane -c "
...
case "$TARGET" in
  api)
    cd "$ROOT_DIR"
    docker compose -f infra/cloud/docker-compose.yml logs --tail="${LOG_LINES:-200}" -f api
    ;;
  local-host)
  ...
  all)
    cd "$ROOT_DIR"
    echo "== API logs =="
    docker compose -f infra/cloud/docker-compose.yml logs --tail="${LOG_LINES:-80}" api || true
    echo
