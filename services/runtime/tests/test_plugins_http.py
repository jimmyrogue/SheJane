...
WORKER_FIXTURE = REPO_ROOT / "plugins" / "fixtures" / "worker-documents"
...
def test_install_wasi_plugin_is_idempotent_and_lists_from_runtime_store(
    ...
    first = client.post("/local/v1/commands", headers=AUTH, json=command)
    ...
    replay = client.post("/local/v1/commands", headers=AUTH, json=command)
    ...
    package.unlink()
    listed = client.get("/local/v1/plugins", headers=AUTH)
...
def test_install_requires_unsigned_confirmation(client: TestClient, tmp_path: Path) -> None:
    ...
    assert client.get("/local/v1/plugins", headers=AUTH).json() == {"plugins": []}
...
def test_install_rejects_same_version_with_different_digest(
    ...
    assert (
        client.get("/local/v1/plugins", headers=AUTH).json()["plugins"][0]["digest"]
...
def test_plugin_detail_enable_and_disable_are_runtime_owned_commands(
    ...
    detail = client.get(
        "/local/v1/plugins/dev.shejane.fixture.archive",
        headers=AUTH,
    )
    ...
    enabled = client.post("/local/v1/commands", headers=AUTH, json=enable_command)
    replay = client.post("/local/v1/commands", headers=AUTH, json=enable_command)
    ...
    assert client.get("/local/v1/plugins", headers=AUTH).json()["plugins"][0]["enabled"] is True
    ...
    assert client.get("/local/v1/plugins", headers=AUTH).json()["plugins"][0]["enabled"] is False
...
def test_vision_model_binding_is_validated_and_frozen_per_run(
    ...
    with TestClient(create_app(settings)) as client:
        ...
        provider = client.put(
            "/local/v1/model-providers/vision",
            headers=AUTH,
            json={
        ...
        detail = client.get(f"/local/v1/plugins/{manifest['id']}", headers=AUTH).json()
...
def test_update_rollback_remove_and_reinstall_preserve_version_history(
    ...
    listed = client.get("/local/v1/plugins", headers=AUTH).json()["plugins"][0]
...
def test_signed_install_uses_deployment_trust_store_and_rejects_revoked_key(
    ...
    assert accepted.status_code == 200, accepted.text
    assert (
        client.get("/local/v1/plugins", headers=AUTH).json()["plugins"][0]["signature_status"]
        == "verified"
    )
