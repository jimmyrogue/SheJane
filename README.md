...
uv sync
JIANDANLY_LOCAL_HOST_TOKEN=dev-local-token uv run python -m local_host
```

> Phase 5'+: 本地 daemon 已切换为 Python + LangGraph create_agent + middleware
> 栈，替代旧 Node/TS runner。客户端 HTTP API 兼容（`/local/v1/*` 路径不变）。
...
启用本地 MCP 通过 `JIANDANLY_LOCAL_MCP_SERVERS` 配置 stdio/HTTP/SSE server（由 `langchain-mcp-adapters` 加载，无需手动维护 allowlist）：

```bash
cd local-host/python
JIANDANLY_LOCAL_HOST_TOKEN=dev-local-token \
JIANDANLY_LOCAL_MCP_SERVERS='{"local-docs":{"command":"node","args":["/absolute/path/to/mcp-server.mjs"],"transport":"stdio"}}' \
...
cd ../local-host/python && uv sync && uv run pytest
```
...
JIANDANLY_CLOUD_TOKEN=用户 access token \
uv run python -m local_host
```
