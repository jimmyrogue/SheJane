...
Phase 2A 已经补齐“文档能力”的底座，Phase 2.1 已把它并入统一 composer。Phase 2.2 进一步把普通聊天和附件问答收敛到同一套 Agent Run 协议。Phase 2.3a-2.13 已新增本地 daemon、Harness loop、artifact/checkpoint/memory、web 工具、stdio MCP runtime、并发安全工具批处理、模型失败 durable handling、client UI bridge、工作区授权治理、本地项目引用、最近 run 恢复和脱敏诊断导出：Electron 会探测 `GET /local/v1/health`，已配对时普通无附件消息会进入本地 run，并在 timeline 中展示权限、artifact 和验证结果。
